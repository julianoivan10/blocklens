...
export default async function PortfolioPage({ searchParams }: { searchParams: Promise<{ period?: string }> }) {
  ...
  let data;
  try {
    const [overview, perf, insight] = await Promise.all([
      getPortfolioOverview(user.id),
      getPerformance(user.id, period),
      getLatestInsight(user.id),
    ]);
    data = { overview, perf, insight };
  } catch (error) {
    logServerError('portfolio:page', error);
    return (
  ...
  const { overview, perf, insight } = data;
